...
export default function ServiceReport() {
    ...
    const [showAttenderSignatureModal, setShowAttenderSignatureModal] = useState(false);
    ...
    const submitReport = async () => {
        setSubmitting(true);
        try {
            const serviceTypeIds = [...selectedServiceTypes, ...selectedServiceStatus].join(',');
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background }]}>
            ...
            <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
                ...
                {/* Ticket Number */}
                <View style={styles.section}>
                    <Text style={[styles.label, { color: theme.text }]}>Ticket number</Text>
                    <View style={[styles.input, { backgroundColor: theme.inputBg, borderColor: theme.inputBorder }]}>
                        <TextInput
                ...
                {/* Service Type */}
                <View style={styles.section}>
                    <Text style={[styles.label, { color: theme.text }]}>Select Service Types</Text>
                    <TouchableOpacity
                        style={[styles.input, { backgroundColor: theme.inputBg, borderColor: theme.inputBorder }]}
                        onPress={() => setShowServiceTypeModal(true)}
                    >
                        <Text style={[styles.inputText, { color: selectedServiceTypes.length > 0 ? theme.text : theme.placeholder }]}>
                            {selectedServiceTypes.length > 0 ? getSelectedServiceTypeNames() : '--Select--'}
                        </Text>
                ...
                <View style={styles.section}>
                    ...
                    <TouchableOpacity
                        style={[styles.input, { backgroundColor: theme.inputBg, borderColor: theme.inputBorder }]}
                        onPress={() => setShowDatePicker({ visible: true, type: 'call', mode: 'date' })}
                    >
                ...
                <View style={styles.section}>
                    ...
                    <TouchableOpacity
                        style={[styles.input, { backgroundColor: theme.inputBg, borderColor: theme.inputBorder }]}
                        onPress={() => setShowDatePicker({ visible: true, type: 'appointment', mode: 'date' })}
                    >
                        <Text style={[styles.inputText, { color: theme.text }]}>
                ...
                <View style={styles.section}>
                    <Text style={[styles.label, { color: theme.text }]}>Arrival Date & Time</Text>
                    <TouchableOpacity
                        style={[styles.input, { backgroundColor: theme.inputBg, borderColor: theme.inputBorder }]}
                        onPress={() => setShowDatePicker({ visible: true, type: 'start', mode: 'date' })}
                    >
                        <Text style={[styles.inputText, { color: theme.text }]}>
                ...
                <View style={styles.section}>
                    <View style={styles.sectionHeader}>
                        <Text style={[styles.label, { color: theme.text }]}>Service Details</Text>
                        <TouchableOpacity
                    ...
                    {serviceDetails.length > 0 && (
                        <View style={styles.serviceDetailsList}>
                            {serviceDetails.map((detail, index) => (
                                <TouchableOpacity
                                    key={index}
                                    style={[styles.serviceDetailItem, { backgroundColor: theme.cardBackground, borderColor: theme.inputBorder }]}
                                    onPress={() => handleEditServiceDetail(index)}
                                >
                                    <View style={styles.serviceDetailContent}>
                                        <Text style={[styles.serviceDetailTitle, { color: theme.text }]}>
                                            Service Description {index + 1}
                                        </Text>
                                        {detail.PartDescC && (
                                            <Text style={[styles.serviceDetailText, { color: theme.placeholder }]} numberOfLines={1}>
                                                {detail.PartDescC}
                                            </Text>
                                        )}
                                        {detail.PartCodeC && (
                ...
                {/* Service Status */}
                <View style={styles.section}>
                    <Text style={[styles.label, { color: theme.text }]}>Select Service Status</Text>
                    <TouchableOpacity
                        style={[styles.input, { backgroundColor: theme.inputBg, borderColor: theme.inputBorder }]}
                        onPress={() => setShowServiceStatusModal(true)}
                    >
                        <Text style={[styles.inputText, { color: selectedServiceStatus.length > 0 ? theme.text : theme.placeholder }]}>
                            {selectedServiceStatus.length > 0 ? getSelectedServiceStatusNames() : '--Select--'}
                        </Text>
                        <Ionicons name="chevron-down" size={20} color={theme.placeholder} />
                    </TouchableOpacity>
                </View>
                ...
                {/* Fault Description */}
                <View style={styles.section}>
                    <Text style={[styles.label, { color: theme.text }]}>Fault Description</Text>
                    <View style={[styles.textAreaContainer, { backgroundColor: theme.inputBg, borderColor: theme.inputBorder }]}>
                        <TextInput
                ...
                {/* Action Taken */}
                <View style={styles.section}>
                    <Text style={[styles.label, { color: theme.text }]}>Action Taken</Text>
                    <View style={[styles.textAreaContainer, { backgroundColor: theme.inputBg, borderColor: theme.inputBorder }]}>
                        <TextInput
                            style={[styles.textArea, { color: theme.text }]}
                            placeholder=""
                ...
                {/* Signatures */}
                <View style={styles.section}>
                    <Text style={[styles.label, { color: theme.text }]}>Attender Signature</Text>
                    <View style={styles.signatureRow}>
                        <View style={[styles.signatureBox, { borderColor: theme.inputBorder }]}>
                            {attendeeSignature ? (
                                <Image
                        ...
                        <TouchableOpacity
                            style={[styles.signButton, { backgroundColor: theme.placeholder }]}
                ...
                <View style={styles.section}>
                    <Text style={[styles.label, { color: theme.text }]}>Client Signature</Text>
                    <View style={styles.signatureRow}>
                        <View style={[styles.signatureBox, { borderColor: theme.inputBorder }]}>
                            {clientSignature ? (
                                <Image
            ...
            <ServiceDetailsModal
                ...
                editingDetail={editingDetailIndex !== null ? serviceDetails[editingDetailIndex] : null}
            />
            ...
            {showDatePicker.visible && showDatePicker.type && (
                <DateTimePicker
                    value={
                        showDatePicker.type === 'call' ? callTime :
                            showDatePicker.type === 'appointment' ? appointmentTime :
                                showDatePicker.type === 'start' ? startTime :
