...
export function AnimatedTabBar({ state, navigation }: BottomTabBarProps) {
  ...
  useEffect(() => {
    curveX.value = withTiming(state.index * tabWidth + tabWidth / 2, {
      duration: 350,
    });
  ...
  const circleStyle = useAnimatedStyle(() => ({
    transform: [
      {
        translateX: curveX.value - tabWidth / 2,
      },
    ],
  }));

  const activeRoute = state.routes[state.index].name;
  const activeIcon = isTabKey(activeRoute) ? iconMap[activeRoute] : "ellipse";
