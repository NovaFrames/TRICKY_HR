...
const RequestModal: React.FC<RequestModalProps> = ({
            ...
            try {
              ...
              if (
                ...
                const getRequestType = (desc: string): string => {
                  ...
                  if (d.includes("leave") || d.includes("permission"))
                    return "Lev";
  ...
  const renderDetailedContent = () => {
      ...
      return (
        <>
          <DetailItem
            icon="document-text-outline"
            label="LEAVE TYPE"
            value={leaveData.ReaGrpNameC || "N/A"}
          />
          <DetailItem
            icon="calendar-outline"
            label="FROM DATE"
            value={formatDate(leaveData.LFromDateD)}
          />
          <DetailItem
            icon="calendar-outline"
            label="TO DATE"
            value={formatDate(leaveData.LToDateD)}
          />
          <DetailItem
            icon="time-outline"
            label="NUMBER OF DAYS"
            value={leaveData.LeaveDaysN?.toString() || "0"}
          />
          ...
          {hasPermission && (
            <>
              <DetailItem
                icon="time-outline"
                label="FROM TIME"
                value={formatTime(leaveData.FHN)}
              />
              <DetailItem
                icon="time-outline"
                label="TO TIME"
                value={formatTime(leaveData.THN)}
              />
              <DetailItem
                icon="hourglass-outline"
                label="TOTAL HOURS"
                value={formatTime(leaveData.THrsN)}
              />
            </>
          )}
          ...
          <DetailItem
            icon="chatbubble-outline"
            label="REMARKS"
            value={leaveData.LVRemarksC || "No remarks"}
          />
          ...
          {leaveData.LVCancelRemarksC && (
            <DetailItem
              ...
              value={leaveData.LVCancelRemarksC}
            />
          )}
        </>
      );
    ...
    if (descLower.includes("surrender") && detailedData.data?.[0]) {
      ...
      return (
        <>
          <DetailItem
            icon="gift-outline"
            label="SURRENDER LEAVES"
            value={surrenderData.SurrenderN?.toFixed(2) || "0"}
          />
          <DetailItem
            icon="calendar-outline"
            label="PAYOUT DATE"
            value={formatDate(surrenderData.PayoutDateD)}
          />
          <DetailItem
            icon="chatbubble-outline"
            label="REMARKS"
            value={surrenderData.RemarksC || "No remarks"}
          />
        </>
      );
    }
    ...
    if (descLower.includes("profile") && detailedData.data?.[0]?.empProfile) {
      ...
      const maritalStatus =
        ["", "Married", "Unmarried", "Divorce", "Others", "Widow"][
          profileData.MaritalN
        ] || "N/A";
      const bloodGroup =
        [
          "",
      ...
      return (
        <>
          <DetailItem
            icon="person-outline"
            label="NAME"
            value={`${profileData.EmpNameC} ${profileData.MiddleNameC} ${profileData.LastNameC}`}
          />
          <DetailItem
            icon="call-outline"
            label="PHONE"
            value={profileData.PhNoC || "N/A"}
          />
          <DetailItem
            icon="mail-outline"
            label="EMAIL"
            value={profileData.EmailIdC || "N/A"}
          />
          <DetailItem
            icon="heart-outline"
          ...
          <DetailItem
            ...
            value={`${profileData.CDoorNoC}, ${profileData.CStreetC}, ${profileData.CAreaC}, ${profileData.CCityC}`}
          />
        </>
      );
    }
        ...
        <>
          ...
          {documents.length > 0 && (
            <View style={styles.documentsSection}>
              <Text
                style={[styles.documentsSectionTitle, { color: theme.text }]}
              >
