...
const LeaveSurrenderModal: React.FC<LeaveSurrenderModalProps> = ({ visible, onClose, item, onRefresh }) => {
                ...
                { text: "No", style: "cancel" },
                {
                    ...
                    onPress: async () => {
                        ...
                        try {
                            ...
                            if (result.success) {
                                Alert.alert("Success", "Leave surrender request cancelled successfully");
                                onClose();
                                onRefresh?.();
                            } else {
                                Alert.alert("Error", result.error || "Failed to cancel leave surrender request.");
                            }
                        } catch (error: any) {
    ...
    const renderContent = () => {
        ...
        if (!surrenderData) {
            return (
                <View style={styles.emptyContainer}>
                    <Text style={[styles.emptyText, { color: theme.placeholder }]}>No surrender details available</Text>
                </View>
            );
        }
        ...
        const displayValues = {
            ...
            remarks: safeString(surrenderData.RemarksC) === 'N/A' ? 'No remarks' : safeString(surrenderData.RemarksC)
        };

        return (
            <View style={styles.content}>
                <View style={styles.infoRow}>
                    <Text style={[styles.infoLabel, { color: theme.textLight }]}>Surrender Leave</Text>
                ...
                <View style={styles.infoRow}>
                    <Text style={[styles.infoLabel, { color: theme.textLight }]}>Payout Date</Text>
                    <Text style={[styles.infoValue, { color: theme.text }]} numberOfLines={2}>
